...
total_collisions = 0  # Contatore globale per le collisioni in una simulazione

def check_for_accidents():
    global accident_occurred, total_collisions
    try:
        ...
        if colliding_pairs:
            ...
            total_collisions += len(colliding_pairs)  # Incrementa il contatore delle collisioni
            for collision in colliding_pairs:
                ...
                if (lane1.startswith(JUNCTION_INTERNAL_PREFIX) and
                    ...
                    # updateAccidents(program_traffic_light, current_time)
...
def run_simulation(seed):
    global accident_occurred, total_collisions, accidents, lane_changed
    ...
    program_traffic_light = "1"  # Fissato a "pericoloso"
    ...
    try:
        ...
        simulation_time = end_time - start_time  # Calcola il tempo impiegato
        return total_collisions, simulation_time
    except Exception as e:
        print(f"Errore generale nella simulazione: {e}")
        if traci.isLoaded():
            traci.close()
        return float('inf'), float('inf')  # In caso di errore, restituisci valori alti per scartare questa simulazione
...
def select_best_seeds():
    ...
    # Esegui più simulazioni con diversi seed e programma semaforico = 1
    for i in range(num_simulations):
        seed = int(time.time()) + i
        print(f"Simulazione {i+1}/{num_simulations} con seed {seed} e programma semaforico 1")
        
        collisions, sim_time = run_simulation(seed)
    ...
    best_data = [(seeds[i], results[i], times[i]) for i in range(len(seeds)) if results[i] <= threshold]
    
    if not best_data:
        print("Nessun caso valido trovato, seleziono i migliori disponibili.")
        # Se non ci sono casi sotto il percentile 25, prendi i migliori disponibili
        sorted_indices = np.argsort(results)
        best_indices = sorted_indices[:max(1, int(0.25 * len(results)))]  # Prendi il 25% migliore
        best_data = [(seeds[i], results[i], times[i]) for i in best_indices]

    # Ordina i migliori in base al tempo di completamento (dal più veloce al più lento)
    best_data.sort(key=lambda x: x[2])  # Ordina per tempo (x[2] è il tempo di simulazione)
    
    # Restituisci la lista dei migliori seed, insieme al numero di collisioni e al tempo
    return [(data[0], data[1], data[2]) for data in best_data]

def main():
    global accident_occurred, program_traffic_light
    
    # Esegui più simulazioni per trovare i migliori seed
    print("Esecuzione di simulazioni per trovare i migliori seed...")
    best_seeds_data = select_best_seeds()
    program_traffic_light = "1"  # Fissato a "pericoloso"

    # Stampa la lista dei migliori seed
    print("\nLista dei migliori seed (ordinati per tempo di completamento):")
    for seed, collisions, sim_time in best_seeds_data:
        print(f"Seed: {seed}, Collisioni: {collisions}, Tempo di simulazione: {sim_time:.2f} secondi")

    # Esegui la simulazione finale con il miglior seed (il più veloce tra i migliori)
    best_seed = best_seeds_data[0][0]  # Prendi il primo seed (il più veloce)
    print(f"\nAvvio della simulazione finale con il miglior seed: {best_seed}")
